...
export const CommunityProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createPost = (
    ...
    const newPost: CommunityPost = {
      ...
      authorAvatar: user?.avatar || `https://api.dicebear.com/7.x/shapes/svg?seed=${user?.username || "dev" + now}`,
  ...
  const addComment = (postId: string, body: string, user?: UserProfile | null) => {
    ...
    const newComment: Comment = {
      ...
      authorAvatar: user?.avatar || `https://api.dicebear.com/7.x/shapes/svg?seed=${user?.username || "dev"}`,
